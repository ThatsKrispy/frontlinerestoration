/* ============================================================
   FRONTLINE RESTORATION — ADA Accessibility Widget
   ============================================================ */
(function () {
  'use strict';

  // Inject widget HTML after footer loads
  function injectWidget() {
    const btn = document.createElement('button');
    btn.id = 'ada-widget-btn';
    btn.setAttribute('aria-label', 'Accessibility options');
    btn.setAttribute('aria-expanded', 'false');
    btn.setAttribute('aria-controls', 'ada-panel');
    btn.innerHTML = `<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><circle cx="12" cy="4" r="2"/><path d="M10 22v-6m4 6v-6m-6-4l2-4h4l2 4m-8 0H6m12 0h-2"/></svg>`;
    ...
    btn.addEventListener('click', () => {
      ...
      if (open) {
        panel.querySelector('.ada-btn').focus();
      }
    });
    ...
    // Control buttons
    panel.querySelectorAll('.ada-btn[data-ada]').forEach(b => {
      b.addEventListener('click', () => {
        const action = b.getAttribute('data-ada');
  ...
  function defaultState() {
    return { font: 'normal', contrast: 'normal', links: 'normal', motion: 'normal', color: 'normal' };
  }

  function loadState() {
    try {
      const s = localStorage.getItem('fr_ada');
      return s ? Object.assign(defaultState(), JSON.parse(s)) : defaultState();
    } catch(e) { return defaultState(); }
  }

  function saveState(s) {
    try { localStorage.setItem('fr_ada', JSON.stringify(s)); } catch(e) {}
  }

  function handleAction(action, state) {
    const s = Object.assign({}, state);
    if (action.startsWith('font-')) s.font = action.replace('font-', '');
    else if (action.startsWith('contrast-')) s.contrast = action.replace('contrast-', '');
    else if (action.startsWith('links-')) s.links = action.replace('links-', '');
    else if (action === 'reduce-motion') s.motion = 'reduce';
    else if (action === 'motion-normal') s.motion = 'normal';
    else if (action === 'grayscale') s.color = 'grayscale';
    else if (action === 'color-normal') s.color = 'normal';
    return s;
  }
  ...
  function applyState(s) {
    ...
    // Font
    b.classList.remove('ada-font-large', 'ada-font-xlarge');
    if (s.font === 'large') b.classList.add('ada-font-large');
    if (s.font === 'xlarge') b.classList.add('ada-font-xlarge');
    // Contrast
    b.classList.remove('ada-contrast-high');
    if (s.contrast === 'high') b.classList.add('ada-contrast-high');
    // Links
    b.classList.remove('ada-links-underline');
    if (s.links === 'underline') b.classList.add('ada-links-underline');
    // Motion
    b.classList.remove('ada-reduce-motion');
    if (s.motion === 'reduce') b.classList.add('ada-reduce-motion');
    // Color
    b.classList.remove('ada-grayscale');
    if (s.color === 'grayscale') b.classList.add('ada-grayscale');
  }

  function markActive(s) {
    const panel = document.getElementById('ada-panel');
    if (!panel) return;
    panel.querySelectorAll('.ada-btn').forEach(b => b.classList.remove('active'));
    const map = {
      [`font-${s.font}`]: true,
      [`contrast-${s.contrast}`]: true,
      [`links-${s.links}`]: true,
      [s.motion === 'reduce' ? 'reduce-motion' : 'motion-normal']: true,
      [s.color === 'grayscale' ? 'grayscale' : 'color-normal']: true,
    };
    panel.querySelectorAll('.ada-btn[data-ada]').forEach(b => {
      if (map[b.getAttribute('data-ada')]) b.classList.add('active');
    });
  }
  ...
  function initConsent() {
    ...
    if (settings) settings.addEventListener('click', () => {
      ...
      settings.textContent = open ? 'Customize' : 'Hide Options';
    });
  ...
  function setConsent(prefs) {
    localStorage.setItem('fr_consent', JSON.stringify({ ...prefs, ts: Date.now() }));
  }

  function loadAnalytics() {
    // Placeholder — swap GA_MEASUREMENT_ID for real ID when ready
    ...
    console.log('[FR] Analytics consent granted');
  }

  // ── Main JS (mobile menu, sliders, counters, etc.) ──
  function initMain() {
    // Mobile menu
    ...
    document.querySelectorAll('.photo-slider').forEach(slider => {
      ...
      const wrap   = slider.closest('.section') || slider.parentElement;
      ...
      let auto = setInterval(() => goto(current + 1 > slides.length - visibleCount() ? 0 : current + 1), 4200);
      slider.addEventListener('mouseenter', () => clearInterval(auto));
      slider.addEventListener('mouseleave', () => {
        auto = setInterval(() => goto(current + 1 > slides.length - visibleCount() ? 0 : current + 1), 4200);
      });
    });
  ...
  document.addEventListener('DOMContentLoaded', () => {
    // Wait a tick for components.js to inject header/footer
    setTimeout(() => {
      injectWidget();
      initConsent();
      initMain();
